...
import post, { Fetch as FetchPackFetch, Auth } from "./post";
...
export default async function fetch(
  ...
  const { capabilities, remoteRefs } = await lsRemote(
    url,
    fetch,
    "git-upload-pack",
    auth
  ...
  const differingRefs = await getRefsToFetch(
    refspec,
    remoteRefs,
    localRefs,
    hasObject
  ...
  if (wanted.length === 0) {
    return {
      ...
      refs: differingRefs.map(ref => ({
        name: ref.name,
        oldHash: ref.localHash,
        hash: ref.remoteHash
      })),
  ...
  const negotiate = negotiatePack(
    wanted,
    localRefs.map(ref => ref.hash),
    shallows,
    unshallow ? 0x7fffffff : depth
  );
  const body = composeWantRequest(negotiate, commonCapabilities(capabilities));
  const response = post(url, "git-upload-pack", body, fetch, auth);
  {
    ...
    return {
      ...
      objects: unpack(
        createResult(response, shallow.resolve, unshallow.resolve, progress),
        progress
      ),
...
async function* createResult(
  ...
  resolveShallow: (v: string[]) => void,
  resolveUnshallow: (v: string[]) => void,
  progress?: Progress
) {
